...
def check_branches(remote_branches, prior_branches)
  remote_branches.each do |full_branch_name|
    #puts "=> #{full_branch_name}"
    ...
    if (branch_name =~ /release-0012\.1\.\w+$/ and repo =='origin')
      ...
      puts "Want me to check on #{repo}/#{branch_name}? (y/n/q)"
      input = gets.strip
      if handle_response(input, 'y', prior_branches)
        #puts "Checking out #{repo}/#{branch_name}"
        checkout_response =  `git checkout -t #{repo}/#{branch_name} 2>&1`
        if (checkout_response =~ /fatal: A branch named '#{branch_name}' already exists/)
          puts "already exists... switching to it"
          checkout_branch(branch_name)
          results = `git pull origin #{branch_name} 2>&1`
          #puts "=> Results of pull #{results}"
        end
        #puts "checkout_response: #{checkout_response}"
        command = nil
        # if (cherry_to_pick.nil?)
        ...
        # puts "=> check if security patch is here"
        # results = `head config/initializers/json.rb ; grep json_pure Gemfile.lock ; grep json Gemfile.lock`
        # puts results
        # puts ""

        # puts "=> check if filter fix is present"
        # results = `grep "Add to Condition" ~/projects/newppm/app/views/filters/_condition.rhtml`
        # puts results
        # puts ""

        puts ""
        puts ">>>>> Find FilledContainer with ack"
        results = `ack FilledContainer`
        puts results
        puts ">>>>> Find FilledContainer with ack"
        puts ""
...
check_branches(remote_branches, prior_branches)
puts "Removing progress file."
if File.exists? PROGRESS_FILE
  File.delete PROGRESS_FILE
end
